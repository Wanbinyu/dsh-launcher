...
> [!NOTE]
> This is an independent convenience wrapper. It does not change the DeepSeek Harness source tree or claim to be an official DeepSeek tool. It is a CLI tool, not a Cordis plugin, and it does not provide a `dsh.bundle`.
...
The installer puts `G:\skill\dsh-launcher` first in the current user's `PATH`. Open a new terminal after installation. The command-file equivalent is `.\install.cmd`.
...
| `DSH_WEB_PORT` | Port used to build the default URL; it must match the Harness server configuration. |
| `DSH_AUTO_OPEN` | Set to `0`, `false`, `no`, or `off` to skip opening the browser. |
...
| Symptom | Check |
| --- | --- |
| `dsh` is not recognized | Open a new terminal after installing and confirm `G:\skill\dsh-launcher` is on the user PATH. |
| The wrong CLI runs | Run `Get-Command dsh -All`; rerun `install.ps1` so the launcher directory is first in the user PATH. |
| The browser opens the wrong port | Set `DSH_WEB_URL` to the exact URL used by Harness. |
| npx is slow on first launch | Install `@deepseek-ai/dsh` globally or set `DEEPSEEK_HARNESS_DIR`. |
| Source mode fails | Confirm the directory contains `package.json`, pnpm is on PATH, and `pnpm dsh` works there. |
...
This repository is a Windows entrypoint wrapper. The Harness CLI, profiles, plugins, configuration, and browser application remain owned by the official project.
