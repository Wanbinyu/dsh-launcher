...
- 兼容 Harness `0.1.2+` 的一次性 Web 登录 Token：从后台启动输出中提取官方认证地址并直接打开，Token 只保存在当前进程内存中。
- 合并桌面、托盘和命令行发出的重复启动请求，只在同一次启动真正就绪后打开一次浏览器。
- EXE、托盘、桌面快捷方式和安装器使用统一的多尺寸 Windows 图标。
- 隐藏运行 Harness 子进程，退出托盘时清理整个子进程树。
- Harness 标准输出和错误输出写入本地日志，便于排查启动失败；Web 登录 Token 和 Bearer 凭据会在写入前自动脱敏。
...
DeepSeek Harness `0.1.2+` 会在启动时输出带一次性 Token 的认证地址。启动器把这行输出作为官方就绪信号，确认它仍属于配置的本机地址并校验协议、主机和端口后，才会打开完整地址；Token 不会显示在状态、诊断报告或日志中。旧版 Harness 仍按原有的 HTTP 就绪检测启动。
